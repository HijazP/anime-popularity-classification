...
## Transformasi data: Mengubah atribut score dan scored_by menjadi w_score (score/score-by)
### Init 
summary(df$score)
min = 0
median = 6.98
...
summary(w_score[])
print(w_score)
### Membuang atribut score dan score_by serta combine kedalam data
df$score <- NULL
df$scored_by <- NULL
df = cbind(w_score,df)


# Diskritisasi variabel target popularity dengan k-mean clustering, k = 2, bin = 3 (populer, cukup populer, dan tidak populer)
library(stats)
k <- 2  # Number of clusters
set.seed(75)
kmeans_result <- kmeans(df$popularity, centers = k)

# Step 2: Get cluster labels
labels <- kmeans_result$cluster
labels
kmeans_result$centers
range <- c(0, kmeans_result$centers, 14469)
df$popularity_category <- cut(df$popularity, breaks = range, labels = c("populer", "cukup_populer", "kurang_populer"))
...
# solusi: normalisasi atribut members atau hilangkan

### Hapus atribut duration
df$duration <- NULL

### Hapus atribut rank
df$rank <- NULL

### Hapus atribut studio
df$studio <- NULL

### Hapus atribut members
df$members <- NULL


write.csv(df, "./dataset/data_clean_final_kmean.csv")
...
# 3.) Data Splitting
## Membaca data
dataset <- read.csv('./dataset/data_clean_final_kmean.csv')
...
library(rpart)
library(caret)
## K-Fold Cross Validation untuk memilih model terbaik (model selection)
ctrl <- trainControl(method="cv", number=10, savePrediction=TRUE)
cv_model <- train(popularity_category ~ ., data = train_data, method = "rpart", trControl = ctrl)
print(cv_model)
plot(cv_model)
# diperoleh model terbaik dengan cp = 0.005926251

## Pembentukan dan pelatihan model
model <- rpart(popularity_category ~ ., data = train_data, cp = 0.005926251)
...
train_data_lc$type <- as.factor(train_data_lc$type)
train_data_lc$source <- as.factor(train_data_lc$source)
train_data_lc$rating <- as.factor(train_data_lc$rating)

library(caret)
set.seed(27)
learning_curve <- learning_curve_dat(dat = train_data_lc, 
                                     outcome = "popularity_category",
                                     test_prop=0.2,
                                     ## `train` arguments:
                                     method = "rpart", 
                                     metric = "Accuracy")

ggplot(learning_curve, aes(x = Training_Size, y = Accuracy, color = Data)) + 
...
library(rpart.plot)
rpart.plot(model)
# Ternyata atribut yang diperlukan hanya "w_score dan favorites" saja
