...
print(cv_model)
plot(cv_model)
# diperoleh model terbaik dengan cp = 0.01364412

## Pembentukan dan pelatihan model
model <- rpart(popularity_category ~ ., data = train_data, cp = 0.01364412)
...
library(rpart.plot)
rpart.plot(model)
# Ternyata atribut yang diperlukan hanya "w_score" saja
